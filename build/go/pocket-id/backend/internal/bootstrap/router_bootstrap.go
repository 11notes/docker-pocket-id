...
func initRouterInternal(db *gorm.DB, svc *services) (utils.Service, error) {
	...
	r := gin.New()
	r.Use(gin.LoggerWithConfig(gin.LoggerConfig{Skip:func(c *gin.Context) bool { return strings.Split(c.Request.RemoteAddr, ":")[0] == "127.0.0.1" }, SkipPaths:[]string{"/api/application-configuration/favicon"}}))
