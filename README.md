...
| ---: | ---: | :---: | :---: | :---: |
| 11notes/pocket-id | 42MB | 1000:1000 | ✅ | amd64, arm64 |
| pocket-id/pocket-id | 90MB | 0:0 | ❌ | amd64, arm64 |
  ...
  pocket-id:
    depends_on:
      postgres:
        ...
        restart: true
    image: "11notes/pocket-id:2.10.0"
    <<: *lockdown
    environment:
      TZ: "Europe/Zurich"
      APP_URL: "${FQDN}"
...
* [2.10.0](https://hub.docker.com/r/11notes/pocket-id/tags?name=2.10.0)
* [2.10.0-unraid](https://hub.docker.com/r/11notes/pocket-id/tags?name=2.10.0-unraid)
* [2.10.0-nobody](https://hub.docker.com/r/11notes/pocket-id/tags?name=2.10.0-nobody)

### There is no latest tag, what am I supposed to do about updates?
It is my opinion that the ```:latest``` tag is a bad habbit and should not be used at all. Many developers introduce **breaking changes** in new releases. This would messed up everything for people who use ```:latest```. If you don’t want to change the tag to the latest [semver](https://semver.org/), simply use the short versions of [semver](https://semver.org/). Instead of using ```:2.10.0``` you can use ```:2``` or ```:2.10```. Since on each new version these tags are updated to the latest version of the software, using them is identical to using ```:latest``` but at least fixed to a major or minor version. Which in theory should not introduce breaking changes.

If you still insist on having the bleeding edge release of this app, simply use the ```:rolling``` tag, but be warned! You will get the latest version of the app instantly, regardless of breaking changes or security issues or what so ever. You do this at your own risk!

# REGISTRIES ☁️
```
docker pull 11notes/pocket-id:2.10.0
docker pull ghcr.io/11notes/pocket-id:2.10.0
docker pull quay.io/11notes/pocket-id:2.10.0
```
...
*created 11.07.2026, 06:06:16 (CET)*